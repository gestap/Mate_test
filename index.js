...
function calculateMenAverageAge(people, century) {
  ...
  return res = men.reduce((sum, curent) => (sum + (curent.died - curent.born)), 0) / men.length
}


function calculateWomenAverageAge(people, withChildren) {

  let motherInc = people.filter(person => person.mother !== null)
  .map(person => person.mother);

  let women = people.filter((person) => {
    return withChildren ? (person.sex === 'f' && motherInc.includes(person.name)): person.sex === 'f'
  })

  return women.reduce((sum, curent) => (sum + (curent.died - curent.born)), 0) / women.length;
...
function calculateAverageAgeDiff(people, onlyWithSon) {
  ...
  let averAge = people.filter(person => personObj[person.name] = person)
  .filter((person) => {
    return person.mother ? (onlyWithSon ? person.sex ==='m' && personObj[person.mother] : personObj[person.mother]): false
  })
  .map(person => person.born - personObj[person.mother].born);

  return averAge.reduce((sum, curent) => (sum + curent)) / averAge.length;  
}
